...
import {
  useAnalyticsCreditRechargePreview,
  useAnalyticsCredits,
  useRechargeAnalyticsCredits,
} from "@/hooks/useAnalyticsCredits";
import { usePlatformSettingsContext } from "@/components/admin/PlatformSettingsProvider";
import { AnalyticsCreditPayNowDialog } from "@/components/admin/analytics/AnalyticsCreditPayNowDialog";
...
export function AnalyticsCreditsRechargePane({
  ...
  const { mutateAsync, isPending: isSubmitting } = useRechargeAnalyticsCredits();
  ...
  } = useAnalyticsCreditRechargePreview(selectedPackId, payDialogOpen);

  function handleOpenPack(packId: string) {
    if (!data?.upiPaymentAvailable) return;
    setSelectedPackId(packId);
    setPayDialogOpen(true);
  }

  function handlePayDialogOpenChange(nextOpen: boolean) {
    setPayDialogOpen(nextOpen);
    if (!nextOpen) {
  ...
  async function handleSubmitPayment() {
    ...
    await mutateAsync(selectedPackId);
  }

  const pendingSubmission = data?.pendingRechargeSubmission ?? null;
  ...
  return (
    <>
          ...
          <SheetBody className="space-y-6">
            {isLoading ? (
              ...
              <>
                ...
                {pendingSubmission ? (
                  <div className="rounded-card border border-status-warning/30 bg-status-warning/5 p-4 text-sm text-text-secondary">
                    <p className="font-medium text-text-primary">{copy.pendingRechargeTitle}</p>
                    <p className="mt-2">
                      {copy.pendingRechargeBody
                        .replace("{amount}", formatCurrency(pendingSubmission.amountInr))
                        .replace("{ref}", pendingSubmission.invoiceNumber ?? "—")}
                    </p>
                  </div>
                ) : null}
                ...
                <div className="space-y-3">
                  ...
                  {!data.upiPaymentAvailable ? (
                    <div className="rounded-card border border-status-warning/35 bg-status-warning/[0.06] px-3.5 py-3 text-sm text-text-secondary">
                    ...
                    <>
                      <div className="grid gap-3">
                        {data.packs.map((pack) => (
                          <div
                            key={pack.id}
                            className="rounded-card border border-border bg-surface-card p-4 transition-colors"
                          >
                      ...
                      <p className="text-xs text-text-muted">{copy.manualPaymentNote}</p>
                    </>
                  )}
                </div>
      ...
      <AnalyticsCreditPayNowDialog
        open={payDialogOpen}
        onOpenChange={handlePayDialogOpenChange}
        copy={copy}
        payNow={preview?.payNow ?? null}
        isLoading={isPreviewLoading}
        loadError={
          isPreviewError
            ? copy.loadFailed
            : preview?.payNow.unavailableReason === "NO_UPI"
              ? copy.paymentNotConfigured
              : null
        }
