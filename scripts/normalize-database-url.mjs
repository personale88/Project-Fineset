 ...
 */
export function deriveDirectUrlFromDatabaseUrl(databaseUrl) {
  try {
...
export function applyDirectUrlNormalization(env = process.env) {
  ...
  if (directUrl) {
    ...
    if (normalized !== directUrl) {
      console.warn(
        "[db] DIRECT_URL used the :6543 pooler — normalized to :5432 for migrations.",
      );
  ...
  if (databaseUrl) {
    ...
    console.warn(
      "[db] DIRECT_URL not set — derived a :5432 session URL from DATABASE_URL for migrations.",
    );
  }
}

export function logDatabaseTarget(label, connectionUrl) {
  try {
